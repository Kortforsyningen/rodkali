#!/usr/bin/python 
#MGL/MTL-oversaettelsesprogram, simlk Oktober 09. 
#Til datafiler i NYT format (MGL-sept 09 og frem)
...
#Oversaettelsesproblematik:
#Vi kan ikke skelne f.eks. 20-00-99999 fra 200-09-9999 !! samme nummerformat!
#Der goeres et ad-hoc forsoeg idet problemet med 5 cifrede loebenr, optraeder primaert i Aarhus 
#f.eks. 2006319065 som giver 10 cifre -> K-63-19065
import sys
import os
...
def Numformat2Pointname(navn): #lidt kluntet skrevet....
	navn=navn.strip()
	try: 
		test=int(navn)
	except:
	...
	if navn[0]!="-":  #hvis ikke vandstandsbraet (- foran for vandstandsbraedder)
		nr=int(navn)
		if 9999999999>=nr>999999999: #saa er vi ihvertfald ude i en situation med 5 cifrede loebenumre!
			HNR=int(nr/10000000)
			SNR=int(nr/100000)- HNR*100 #mellemste  (som regel 2)
			LBN=nr%100000
		elif nr<=520000000:
		...
		else: #saa ugyldigt nummer
			return "FEJL:"+navn,0
		if ( nr < 100000): #saa hjaelpepunkt 
			LNSNR ="%5d" %nr
		elif HNR<300: 
			if ( HNR == 200 ):
				HNR = "K"
			LNSNR = "%3s-%02d-%05d" %(HNR,SNR,LBN) #er det OK med 5 cifre i lbnr altid?
			...
			if (HNR<510): 
				...
				return "Fejl:"+navn,0
	else:  #ellers har vi fundet et minus... Her antager vi hoejst 9 cifre!
...
def main(args):
	...
	print msg
	for line in hvd:
		fra,nbadd=Numformat2Pointname(line[0])
		nbadnames+=not(nbadd)
		til,nbadd=Numformat2Pointname(line[1])
		nbadnames+=not(nbadd)
		dato=line[2]
		tid=line[3]
		afst=line[4]
